...
import type { GameRepository, RoomRepository } from "@server/db/repositories";
 ...
 * ### gamesRouter
 *
 * Creates endpoints for creating and editing games.
 ******************************************************************************/
export const gamesRouter = {
  ...
  create: (deps: Dependencies) => {
     ...
     * created game id.
     *
     * Only the room's "host" can start a game. Since none of the room's
     * "guests" should be able to see the host's id, the host's id used as a
    ...
    router.post(
      ...
      validated({
        ...
        handler: async (req, res) => {
          const { rooms: roomRepo, games: gameRepo } = deps.repositories;
     ...
     * ### GET games/
     *
     * Lists active game ids, and the last time each was updated.
     */
    router.get(
      ...
      validated({
        ...
        handler: async (_, res) => {
          const { games: gameRepo } = deps.repositories;
